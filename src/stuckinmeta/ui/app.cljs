  ...
  [:<>
   ...
   [:section.hero {:tw "bg-#001836 flex w-screen items-start min-h-screen"}
    [:img {:src "/images/hero.png"
           :tw "max-h-screen sticky object-contain shrink top-0 min-w-0"}]
    [:div {:tw "p-12 space-y-12 max-w-55em grow"}
     [:h1 {:tw "text-#00fffe font-size-3em"} "stuck" [:br] "_in" [:br] "//meta"]
     [:section {:tw "text-#fda558 space-y-6 text-sm"}
      [:p
     ...
     [:section {:tw "text-#fc40da space-y-6 text-sm"}
     ...
     [:button {:tw "border border-4 border-#00fffe text-#00fffe text-xl p-4"} "> Jack In"]]]
   #_[:section.setting
